...
Discourse::Application.routes.draw do
  ...
  scope path: nil, constraints: { format: %r{(json|html|\*/\*)} } do
    ...
    get "custom_sso_login" => "static#custom_sso_login", :id => "custom_sso_login"
    get "custom_user_pwd_change" => "static#custom_user_pwd_change", :id => "custom_user_pwd_change"
    # get '/*path' => 'static#custom_redirect', constraints: lambda { |request| !request.path.end_with?('.json') || !request.path.include?('/preferences/' ) || !request.path.include?('/custom_sso_login/') || !request.path.include?('/t/' ) || !request.path.include?('/admin/') || !request.path.include?('/signup') || !request.path.include?('/login')}

    get '/*path' => 'static#custom_redirect', constraints: lambda { |request|
      puts "Request Path: #{request.path}"
      !request.path.end_with?('.json') &&
      !request.path.include?('/preferences/') &&
      !request.path.include?('/custom_sso_login/') &&
